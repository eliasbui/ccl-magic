// Package logo renders a Claude Code LOL Magic wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the Claude Code LOL Magic title art.
type Opts struct {
	FieldColor   color.Color // diagonal lines
	TitleColorA  color.Color // left gradient ramp point
	TitleColorB  color.Color // right gradient ramp point
	CharmColor   color.Color // CLAUDE CODE LOL MAGIC text color
	VersionColor color.Color // Version text color
	Width        int         // width of the rendered logo, used for truncation
}

// Render renders the Claude Code LOL Magic logo. Set the argument to true to render the narrow
// version, intended for use in a sidebar.
//
// The compact argument determines whether it renders compact for the sidebar
// or wider for the main pane.
func Render(version string, compact bool, o Opts) string {
	const charm = " CLAUDE CODE LOL MAGIC"
...
// SmallRender renders a smaller version of the CCL-MAGIC logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("CLAUDE CODE LOL MAGIC")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("CCL-MAGIC", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "CCL-MAGIC"
	if remainingWidth > 0 {
...
func letterC(stretch bool) string {
	...
	right := heredoc.Doc(`
		...
		▀  ▀
	`)
	return joinLetterform(left, right)
}
...
// letterDash renders a dash/hyphen.
func letterDash(stretch bool) string {
	// Here's what we're making:
	//
	//
	// ▀▀▀▀
